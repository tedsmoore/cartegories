...
|---|---|---|
| DecksScreen | DeckCollectionViewController | Missing deck purchase modal, full store integration. Back-nav is broken when reached from CardSelection gear icon. |
...
| SettingsScreen | SettingsViewController | Sound toggle works; missing full settings table, more preference options |
...
- **DecksScreen back-nav broken** — gear icon on CardSelection navigates to Decks, but back button doesn't return to CardSelection
